...
def train_awa_vae(dataset="gym_fetch", z_dim=64, batch_size=32, num_epochs=250, beta_kl=1.0, beta_rec=0.0, beta_task=1.0, weight_cross_penalty=0.1, device=0, save_interval=5,
                  lr=2e-4, seed=0, model_path=None, dataset_dir=None, vae_model="CNNBasedVAE", norm_sample=True, rand_crop=False, randpca=False, histepoch=0, view='2image'):
    ### set paths
    if norm_sample:
        model_type = "DVAE"
    else:
    ...
    if dataset == "gym_fetch":
        ...
        if view == 'side':
            print("Side view 1")
            LOG_DIR = LOG_DIR.replace("randPCA", "side-view1")   
            LOG_DIR = LOG_DIR.replace("NoPCA", "side-view1")   
            fig_dir = fig_dir.replace("randPCA", "side-view1")
            fig_dir = fig_dir.replace("NoPCA", "side-view1")
            model_path = model_path.replace("randPCA", "side-view1")
            model_path = model_path.replace("NoPCA", "side-view1")
            obs2 *= 0
        elif view == 'arm':
            print("Arm view 2")
            LOG_DIR = LOG_DIR.replace("randPCA", "arm-view2")   
            LOG_DIR = LOG_DIR.replace("NoPCA", "arm-view2")   
            fig_dir = fig_dir.replace("randPCA", "arm-view2")
            fig_dir = fig_dir.replace("NoPCA", "arm-view2")
            model_path = model_path.replace("randPCA", "arm-view2")
            model_path = model_path.replace("NoPCA", "arm-view2")
            obs1 *= 0
        else:
    ...
    for ep in range(histepoch, num_epochs+histepoch):
        ...
        if (ep + 1) % save_interval == 0 or (ep + 1) == 10 or ep == 0:
            success_rate = evaluate(task_model, DVAE_awa, device, dataset, vae_model, DPCA_tf=False, 
                                    dpca_dim=0, num_episodes=100, view_from=view)[3]
...
if __name__ == "__main__":
    """        
    python train_single_view.py --dataset Lift --device 0 --lr 1e-4 --num_epochs 3000 --beta_rec 0.0 --beta_kl 0.0 --beta_task 500 --z_dim 24 --batch_size 128 --seed 0 --cross_penalty 0.0 --vae_model ResBasedVAE --norm_sample False --rand_crop True -v side
    """
    ...
    parser.add_argument("-v", "--view", type=str, help="what view to train", default="2image")
    ...
    train_awa_vae(dataset=args.dataset, z_dim=args.z_dim, batch_size=args.batch_size, num_epochs=args.num_epochs, weight_cross_penalty=args.cross_penalty, 
                  ...
                  randpca=args.randpca, histepoch=args.histepoch, view=args.view)
