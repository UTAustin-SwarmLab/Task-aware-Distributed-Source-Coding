...
class FetchLiftEnv(FetchPickAndPlaceEnv):
    def __init__(self):
        super().__init__()
...
class PixelLiftEnv(gym.Env):
    def __init__(self, cameras, from_pixels=True, height=128, width=128, channels_first=True):
        ...
        self._env = gym.make('FetchLift-v1')
...
def register_env():
    register(
        id="FetchLift-v1",
        entry_point=FetchLiftEnv,
        max_episode_steps=50,
        reward_threshold=50
    )
    register(
        id="Lift-both-v1",
        entry_point=PixelLiftEnv,
        max_episode_steps=50,
        reward_threshold=50,
        kwargs={'cameras': (8, 10)}
    )
